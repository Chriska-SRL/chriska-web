...
import { useUploadProductImage, useDeleteProductImage } from '@/hooks/product';
...
export const ProductImageUpload = ({ product, onImageChange, editable = false }: ProductImageUploadProps) => {
  ...
  const { data: uploadData, isLoading: isUploading, error: uploadError } = useUploadProductImage(uploadProps);
  const { data: deleteData, isLoading: isDeleting, error: deleteError } = useDeleteProductImage(deleteImageId);
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploadProps({ id: product.id, file });
  };
