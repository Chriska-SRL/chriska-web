...
export const PurchaseFilters = ({ onFilterChange, disabled = false }: PurchaseFiltersProps) => {
  ...
  }, [selectedSupplier, invoiceNumber, fromDate, toDate, onFilterChange]);

  const hasActiveFilters = selectedSupplier !== null || invoiceNumber !== '' || fromDate !== '' || toDate !== '';

  const activeSelectFilters = [invoiceNumber !== '' ? 1 : 0, fromDate !== '' ? 1 : 0, toDate !== '' ? 1 : 0].reduce(
    (a, b) => a + b,
    0,
  );
